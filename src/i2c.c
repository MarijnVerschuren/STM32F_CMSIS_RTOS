...
void fconfig_I2C(I2C_GPIO_t scl, I2C_GPIO_t sda, uint16_t own_address, I2C_address_t address_type, uint8_t dual_address) {
	...
	fconfig_GPIO(scl_port, scl_pin, GPIO_alt_func, GPIO_no_pull, GPIO_open_drain, GPIO_very_high_speed, scl_af);
	fconfig_GPIO(sda_port, sda_pin, GPIO_alt_func, GPIO_no_pull, GPIO_open_drain, GPIO_very_high_speed, sda_af);
...
/*!< master input / output */
uint32_t I2C_master_address(I2C_TypeDef* i2c, uint8_t i2c_address, uint32_t timeout) {
	uint64_t start = tick;
	while (i2c->SR2 & I2C_SR2_BUSY) { if (tick - start > timeout) { return 0; } }  // wait until idle
	if (I2C_start_master(i2c, i2c_address << 1, timeout - (tick - start))) { return -1; }
	I2C_stop_master(i2c);
	return 0;
}
uint32_t I2C_master_write(I2C_TypeDef* i2c, uint8_t i2c_address, const uint8_t* buffer, uint32_t size, uint32_t timeout) {
	uint64_t start = tick;
	while (i2c->SR2 & I2C_SR2_BUSY) { if (tick - start > timeout) { return 0; } }  // wait until idle
	if (I2C_start_master(i2c, i2c_address << 1, timeout - (tick - start))) { return 0; }
...
uint32_t I2C_master_read(I2C_TypeDef* i2c, uint8_t i2c_address, uint8_t* buffer, uint32_t size, uint32_t timeout) {
	uint64_t start = tick;
	while (i2c->SR2 & I2C_SR2_BUSY) { if (tick - start > timeout) { return 0; } }  // wait until idle
	if (I2C_start_master(i2c, (i2c_address << 1) | 0b1ul, timeout - (tick - start))) { return 0; }
...
/*!< slave input / output */
uint32_t I2C_slave_address(I2C_TypeDef* i2c, uint8_t i2c_address, uint32_t timeout) {
	uint64_t start = tick;
	if (I2C_start_master(i2c, i2c_address << 1, timeout - (tick - start))) { return -1; }
	I2C_stop_master(i2c);
	return 0;
